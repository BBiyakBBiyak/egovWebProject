...
<script type="text/javaScript">
	function select_board(unq) {		
		var url = "dataBoardSelect.do?unq=" + unq;
	...
	function fn_action(value) {
		...
		// 추후 정규식 체크 함수 필요
		if(value == 'W') {
			location.href = "<c:url value='/dataBoardWrite.do'/>";
		}	
	}
    ...
    function link_search() {
    	var b = document.board;
    	b.pageIndex.value = 1;
		b.action = "<c:url value='/dataBoardList.do'/>";
       	b.submit();
    }
...
<body>
    ...
    <div class="bg-faded p-4 my-4">
	    <form id="pageVO" name="board">
	  		<hr class="divider">
	        <h2 class="text-center text-lg text-uppercase my-0"><strong>Board List</strong></h2>
	        <hr class="divider">
				...
				<table class="table table-hover table-bordered">
					...
					<c:forEach var="list" items="${result}" varStatus="status">
						<tr>
							...
							<td align="center">
								<c:set var="file" value="${fn:split(list.filename,'／')}" />
								<c:forEach var="fileName" items="${file}" varStatus="g">
									<c:if test="${fileName != '' && fileName !='／'}">
										<div>
											<a href="" onclick="window.open(encodeURI('<c:url value='/downloadFile.do?'/>requestedFile=${fileName}'))">
											<i class="fa fa-download" style="color:green" ></i></a>
												<c:set var="filecutName" value="${fn:split(fileName,'_')}" />
											<font style="font-size: 10pt"><c:out value="${filecutName[1]}"/></font>	
